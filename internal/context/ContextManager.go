...
type (
	...
	ConfigMapType       map[string][]config.Config
...
func (context *ContextManager) loadConfigs() (ConfigMapType, ConfigMapType, error) {
	...
	err := filepath.WalkDir(context.ContextPath, func(path string, d fs.DirEntry, err error) error {
		...
		if !d.IsDir() {
			...
			if section != constants.PackageDirName && section != constants.AppDirName {
				return nil
			}
			...
			if section == constants.PackageDirName {
...
// all Packages/Apps.
func getConfigsArray(platformString *bacpack_package.PlatformString, configMap *ConfigMapType) []*config.Config {
...
func (context *ContextManager) GetAllPackagesStructs(platformString *bacpack_package.PlatformString) ([]bacpack_package.Package, error) {
	...
	var packages []bacpack_package.Package
...
func (context *ContextManager) GetPackageConfigs(packageName string) ([]config.Config, error) {
	...
	if !exists {
		return []config.Config{}, fmt.Errorf("Package/App %s does not exist, please check the name", packageName)
	}
...
func (context *ContextManager) getAllDepConfigs(cfg config.Config, visited map[string]struct{}) ([]config.Config, error) {
	visited[cfg.Package.GetShortPackageName()] = struct{}{}
	addedPackages := 0
	var configListWithDeps []config.Config
	for _, packageDep := range cfg.DependsOn {
		packageDepConfigs, err := context.GetPackageConfigs(packageDep)
		if err != nil {
			return []config.Config{}, fmt.Errorf("cant't get Config of %s package", packageDep)
		}
		for _, depConfig := range packageDepConfigs {
			if depConfig.Package.IsDebug != cfg.Package.IsDebug {
				continue
			}
			addedPackages++
	...
	if addedPackages < len(cfg.DependsOn) {
		return []config.Config{}, fmt.Errorf("package %s dependencies do not have package with same build type", cfg.Package.Name)
	}
...
func (context *ContextManager) getAllDepOnConfigs(cfg config.Config, visited map[string]struct{}, recursively bool) ([]config.Config, error) {
	packConfigs := context.GetAllConfigsArray(nil)
	visited[cfg.Package.Name] = struct{}{}
	var packsToBuild []config.Config
	for _, packConfig := range packConfigs {
		if packConfig.Package.Name == cfg.Package.Name ||
			packConfig.Package.IsDebug != cfg.Package.IsDebug {
			continue
		}
		for _, dep := range packConfig.DependsOn {
			if dep == cfg.Package.Name {
...
// recursively is set.
func (context *ContextManager) addDependsOnPackagesToBuild(packsToBuild *[]config.Config, packConfig *config.Config, visited map[string]struct{}, recursively bool) error {
...
func (context *ContextManager) GetPackageWithDepsConfigs(packageName string) ([]config.Config, error) {
	...
	if err != nil {
		return []config.Config{}, fmt.Errorf("can't get Configs for Package '%s' - %w", packageName, err)
	}
	var packageDeps []config.Config
	visitedPackages := make(map[string]struct{})
	for _, cfg := range packageConfigs {
		packageDepsTmp, err := context.getAllDepConfigs(cfg, visitedPackages)
		if err != nil {
			return []config.Config{}, err
		}
...
func (context *ContextManager) GetPackageWithDepsOnConfigs(packageName string, recursively bool) ([]config.Config, error) {
	...
	visitedPackages := make(map[string]struct{})
	for _, cfg := range packageConfigs {
		packageDepsTmp, err := context.getAllDepOnConfigs(cfg, visitedPackages, recursively)
		if err != nil {
			return []config.Config{}, err
		}
...
func removeConfigs(list1 []config.Config, list2 []config.Config) []config.Config {
...
func removeConfig(list1 []config.Config, config config.Config) []config.Config {
	i := 0
...
func (context *ContextManager) loadImagesDockerfilePaths() error {
	imageDir := path.Join(context.ContextPath, constants.DockerDirName)
...
func (context *ContextManager) validateContextPath() error {
	...
	dockerDirPath := path.Join(context.ContextPath, constants.DockerDirName)
	...
	packageDirPath := path.Join(context.ContextPath, constants.PackageDirName)
	...
	appDirPath := path.Join(context.ContextPath, constants.AppDirName)
